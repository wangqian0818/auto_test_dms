...
def print_info(msg, indent=0):
    ...
    if indent == 0:
        for header in ['From', 'To', 'Cc', 'Subject']:
            ...
            log.warning('%s%s: %s' % ('  ' * indent, header, value))
    if (msg.is_multipart()):
        parts = msg.get_payload()
        for n, part in enumerate(parts):
            log.warning('%spart %s' % ('  ' * indent, n))
            log.warning('%s--------------------' % ('  ' * indent))
        ...
        if content_type == 'text/plain' or content_type == 'text/html':
            ...
            log.warning('%sText: %s' % ('  ' * indent, content + '...'))
            mail_list.append(content)
        else:
            log.warning('%sAttachment: %s' % ('  ' * indent, content_type))
...
if __name__ == "__main__":
    # 获取邮箱密码和对应邮箱POP3服务器
    email = "autotest_recv@jusontech.com"
    password = "1q2w3e"
    # pop3_host = "10.10.101.33"
    # pop3_port = 110
    pop3_host = "192.168.30.47"
    pop3_port = 8886
    msg = get_email(email, password, pop3_host, pop3_port)
    log.warning('---------------------------------------------------------------')
    log.warning(msg)
    log.warning('---------------------------------------------------------------')
    mail_list = print_info(msg)  # 解析
    log.warning('0000000000000000000000000000000000000000000000000000000')
