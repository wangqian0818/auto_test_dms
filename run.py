...
def main():
    ...
    for mode in mode_list:
        ...
        for i in case_name:  # 判断所选择的单元测试例目录是否真实存在,如果不存在就直接报错,结束程序运行
            ...
            if os.path.exists(base_path + '/Case_' + str(mode) + '/' + str(i) + '/function.py'):
                ...
                log.warning('单元测试例目录: ' + str(i) + ' 不存在!请检查后重试.')
                sys.exit(0)  # 避免程序继续运行造成的异常崩溃,友好退出程序
    ...
    try:
        ...
        if not isinstance(cirNum, int) or 0 >= cirNum:
            log.warning("程序友好退出，请输入整数。")
            sys.exit(0)
    except Exception as err:
        log.warning("程序友好退出，请输入整数({})".format(err))
        sys.exit(0)
    log.warning('循环{}次'.format(cirNum))
    log_dirs = []  # 此次循环产生的日志路径
    exist_faileds = []  # 存在failed的case文件夹名称
    for num in range(cirNum):
        log.warning('================================== 开始第{}次循环 =================================='.format(num + 1))
        # 验证并创建log保存目录
        ...
        if not os.path.exists(base_path + '/Logs/' + str(version) + '/' + current_time):  # 如果保存当前版本的logs目录不存在,就创建
            ...
            if msg.lower() == 'y':
                ...
                log.warning('已选择结束程序运行!如需查看已存在的Logs文件,请进入: ' + base_path + '/Logs/' + str(version) + ' 目录查看对应的Log文件')
                sys.exit(0)  # 避免程序继续运行造成的异常崩溃,友好退出程序
            ...
            try:
                try:
                    ...
                    for i in case_list:
                        ...
                        log_path = left + c + right + log_name + time.strftime('%Y%m%d_%H-%M-%S',
                                                                               time.localtime(time.time())) + right
                        abs_path = os.path.dirname(os.path.abspath(__file__))
                        # log.warning('abs_path: ' + abs_path)
                        ...
                        open(log_txt_path + log_path + '.txt', 'a+')
                        os.system('pytest ' + i + ' --html=' + abs_path + '/Logs/' + str(version) + '/' + current_time
                                  + '/' + mode + '/' + log_path + '.html --self-contained-html -o log_file=' + log_txt_path
                                  + log_path + '.txt')
                        file.write("\n--------------------- 执行结束 case：%s\n\n\n" % i)
                        file.flush()
                        ...
                        src_log = log_dir + log_path + '.html'
                        des_log = log_dir + log_path + result + '.html'
                    ...
                    log.warning('执行Pytest测试失败!错误信息如下:')
                    log.warning(err1)
                    sys.exit(0)  # 避免程序继续运行造成的异常崩溃,友好退出程序
            except Exception as error:
                log.warning('pytest测试调用失败!错误内容如下:')
                log.warning(error)
                sys.exit(0)  # 避免程序继续运行造成的异常崩溃,友好退出程序

    log.warning('自动化测试完成！')
    if 0 != len(log_dirs):
        # log.warning("日志路径为：", log_dirs)
        if 0 == len(exist_faileds):
            log.warning('========================== 恭喜，此次循环执行用例{}次，全部成功！！！棒棒~'.format(cirNum))
        else:
            log.warning(
                '========================== 此次循环执行用例{}次，失败{}次，存在失败用例的log文件夹如下: \n{}'.format(cirNum, len(exist_faileds),
                                                                                            exist_faileds))
        return log_dirs
    else:
        log.warning("========================== 日志路径不存在，测试报告尚未生成 ========================== ")
        return None
...
def get_result(result_file, case_path):
    ...
    # skip_num = 0
    # fail_num = 0
    # log.warning("-------------result_list:",result_list)
