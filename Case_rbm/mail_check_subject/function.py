...
class Test_mail_check_subject():
    ...
    @allure.feature('验证基于主题关键字过滤的邮件策略【单个黑名单】')
    def test_mail_check_subject_a1(self):
        ...
        fun.send(rbmExc, tool.interface().app_safe_policy(prototype='mailcheck', subject_data=self.case1_title1),
                 rbmDomain,base_path)
        fun.wait_data('ps -ef |grep nginx', 'gw', 'nginx: worker process')
    ...
    # @pytest.mark.skip(reseason="skip")
    @allure.feature('验证基于多个主题关键字过滤的邮件策略【多个黑名单】')
    def test_mail_check_subject_a2(self):
        ...
        fun.send(rbmExc,
                 tool.interface().app_safe_policy(prototype='mailcheck',
                                                subject_data=f'{self.case2_title1};{self.case2_title2}'),
                 rbmDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'gw', 'nginx: worker process')
        add_check = fun.nginx_worker('ps -ef |grep nginx', 'gw', 'nginx: worker process')
        assert add_check == 1
