...
'''
脚本一：
用例名称：验证含type1标记策略的DOI的规则
编写人员：刘超
编写日期：2021/11/12
测试目的：验证含type1标记策略的DOI的规则
测试步骤：
...
try:
    from Case_rbm.cipso_type1_doi import index
...
class Test_cipso_type1_doi():
    ...
    @allure.feature('用例一：验证含type1标记策略的DOI的规则')
    def test_cipso_doi(self):
        log.warning('用例一：验证含type1标记策略的DOI的规则')
        # 设备设置DOI为16
    ...
    @allure.feature('用例二：验证type1类型下的DOI左边界（最小值）')
    def test_cipso_doi_minimum(self):
        log.warning('用例二：验证type1类型下的DOI左边界（最小值）')
        # 设备设置DOI为1
        # 客户端发送DOI为1的报文
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    # @allure.feature('用例三：验证type1类型下的DOI右边界（最大值）')
    # def test_cipso_doi_maximum(self):
    #     log.warning('用例三：验证type1类型下的DOI右边界（最大值）')
    #     # 设备设置DOI为1
    #     # 客户端发送DOI为1的报文
    #     # 接口名称，过滤规则，抓包数量，报文命名
