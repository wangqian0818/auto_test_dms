...
'''
脚本一：
用例名称：验证网关设备将type1标记和type7标记互相转换的情况
编写人员：刘超
编写日期：2021/11/12
测试目的：验证网关设备将type1标记和type7标记互相转换的情况
测试步骤：
...
脚本二：
用例名称：验证网关设备会话为type1标记，反向会话为type7标记时的情况
编写人员：刘超
编写日期：2021/11/12
...
try:
    from Case_rbm.cipso_type1_type7_markaction import index
...
class Test_cipso_type1_type7_markaction():
    ...
    def change_check_labelStr(self,label):
        """
    ...
    @allure.feature('用例一：验证网关设备将type1标记和type7标记互相转换的情况')
    def test_cipso_type1_type7_change(self):
        log.warning('用例一：验证网关设备将type1标记和type7标记互相转换的情况')
        # 设备设置DOI为1
        # 客户端发送DOI为1的报文
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例二：验证网关设备会话为type1标记，反向会话为type7标记时的情况')
    def test_cipso_type1_type7_rechange(self):
        log.warning('用例二：验证网关设备会话为type1标记，反向会话为type7标记时的情况')
        # 设备设置DOI为1
        # 客户端发送DOI为1的报文
        # 接口名称，过滤规则，抓包数量，报文命名
