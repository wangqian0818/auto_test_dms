...
class Test_mail_check_file():
    ...
    @pytest.mark.skip(reseason="skip")      # 新管控缺少针对文件名的过滤，已提需求_88
    ...
    def test_mail_check_file_a1(self):
        ...
        fun.send(rbmExc,
                 tool.interface().app_safe_policy(prototype='mailcheck', attachmentExt_data=f'{self.case1_filename}'),
                 rbmDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'gw', 'nginx: worker process')
    ...
    def test_mail_check_file_a2(self):
        ...
        fun.send(rbmExc, tool.interface().app_safe_policy(prototype='mailcheck',
                                                        attachmentExt_data=f'{self.case2_filename};{self.case2_deny_file}'),
                 rbmDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'gw', 'nginx: worker process')
        add_check = fun.nginx_worker('ps -ef |grep nginx', 'gw', 'nginx: worker process')
        assert add_check == 1
