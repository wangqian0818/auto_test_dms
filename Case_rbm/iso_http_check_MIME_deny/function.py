...
try:
    from Case_rbm.iso_http_check_MIME_deny import index
...
class Test_iso_http_check_MIME_deny():
    ...
    def test_iso_http_check_MIME_deny_a1(self):
        ...
        # 数据检查
        log.warning('2、下发http的MIME黑名单，参数为：pdf，等待nginx的24个进程起来;预期cat /etc/jsac/http.json文件应该包含：s_content_type和MIME黑名单参数：pdf')
        fun.send(rbmExc, tool.interface().app_safe_policy(prototype='httpcheck', mime_action=1, mime_data=self.check1_uri),
        ...
        log.warning('5、控制台发送get请求，请求内容包含MIME黑名单：pdf；请求失败，状态码返回为403')
    ...
    def test_iso_http_check_MIME_deny_a2(self):
        ...
        # 数据检查
        log.warning(
            '2、下发http的MIME黑名单，参数为：js、mps、gif、tsv、avi，等待nginx的24个进程起来;cat /etc/jsac/http.json文件应该包含：s_content_type和MIME黑名单参数：js、mps、gif、tsv、avi')
        fun.send(rbmExc, tool.interface().app_safe_policy(prototype='httpcheck', mime_action=1,
        ...
        log.warning('5、控制台发送get请求，请求内容包含MIME黑名单：js；请求失败，状态码返回为403')
        ...
        log.warning('6、控制台发送get请求，请求内容包含MIME黑名单：mps；请求失败，状态码返回为403')
        ...
        log.warning('7、控制台发送get请求，请求内容包含MIME黑名单：gif；请求失败，状态码返回为403')
        ...
        log.warning('8、控制台发送get请求，请求内容包含MIME黑名单：tsv；请求失败，状态码返回为403')
        ...
        log.warning('9、控制台发送get请求，请求内容包含MIME黑名单：avi；请求失败，状态码返回为403')
    ...
    def test_iso_http_check_MIME_deny_a3(self):
        ...
        # 数据检查
        log.warning('2、下发http的MIME黑名单，参数为：所有MIME类型，等待nginx的24个进程起来;cat /etc/jsac/http.json文件应该包含：s_content_type和MIME所有类型')
        ...
        data = "evy;fif;spl;hta;acx;hqx;doc;dot;*;bin;class;dms;exe;lha;lzh;oda;axs;pdf;prf;p10;crl;ai;eps;ps;rtf;" \
               ...
               "m14;mvb;wmf;mny;pub;scd;trm;wri;cdf;nc;pma;pmc;pml;pmr;pmw;p12;pfx;p7b;spc;p7r;p7c;p7m;p7s;sh;shar;" \
               "swf;sit;sv4cpio;sv4crc;tar;tcl;tex;texi;texinfo;roff;t;tr;man;me;ms;ustar;src;cer;crt;der;pko;zip;au;" \
               "snd;mid;rmi;mps;aif;aifc;aiff;m3u;ra;ram;wav;bmp;cod;gif;ief;jpe;jpeg;jpg;jfif;svg;tif;tiff;ras;cmx;" \
               "ico;pnm;pbm;pgm;ppm;rgb;xbm;xpm;xwd;mht;mhtml;mws;css;323;htm;html;stm;uls;bas;c;h;txt;rtx;sct;tsv;htt;" \
               "htc;etx;vcf;mp2;mpa;mpe;mpeg;mpg;mpv2;mov;qt;lsf;lsx;asf;asr;asx;avi;movie;flr;vrml;wrl;wrz;xaf;xof"

        fun.send(rbmExc, tool.interface().app_safe_policy(prototype='httpcheck', mime_action=1, mime_data=data), FrontDomain,
                 base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        add_res2 = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        assert add_res2 == 1
