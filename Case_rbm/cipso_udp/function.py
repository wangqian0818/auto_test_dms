...
'''
脚本一：
用例名称：验证含type1标记对UDP报文的去标
编写人员：刘超
编写日期：2021/11/12
测试目的：验证含type1标记对UDP报文的去标
测试步骤：
1.设备下发type1-udp的标记策略，通过babbitMQ下发
2.客户端tcpreplay发送type1的udp报文
3.服务端抓取并分析报文
4.设备移除标记策略，通过babbitMQ删除
预期结果：
1.设备策略下发成功，可以使用tupleacl --get查看
2.客户端tcpreplay发送type1的udp报文成功
3.服务端抓取分析成功
6.设备移除策略成功，可以使用tupleacl --get查看

脚本二：
用例名称：验证含type7标记对UDP报文的去标
编写人员：刘超
编写日期：2021/11/12
测试目的：验证含type7标记对UDP报文的去标
测试步骤：
1.设备下发type7-udp的标记策略，通过babbitMQ下发
2.客户端tcpreplay发送type7的udp报文
3.服务端抓取并分析报文
4.设备移除标记策略，通过babbitMQ删除
预期结果：
1.设备策略下发成功，可以使用tupleacl --get查看
2.客户端tcpreplay发送type7的udp报文成功
3.服务端抓取分析成功
6.设备移除策略成功，可以使用tupleacl --get查看
'''
try:
...
try:
    from Case_rbm.cipso_udp import index
...
class Test_cipso_udp():
    ...
    @allure.feature('用例一：验证含type1标记对UDP报文的去标')
    def test_cipso_udp_type1(self):
        log.warning('用例一：验证含type1标记对UDP报文的去标')
        # 接口名称，过滤规则，抓包数量，报文命名
        ...
        # 设备下发标记策略
        fun.send_cipso(tool, rbmDomain, rbmExc, clientOpeIp, serverOpeIp, mtLabel=self.case1_mtLabel, L4protocol='udp')

        # 服务端抓取报文
        log.warning('验证客户端发送含type1标记的udp报文')
    ...
    @allure.feature('用例二：验证含type7标记对UDP报文的去标')
    def test_cipso_udp_type7(self):
        log.warning('用例二：验证含type7标记对UDP报文的去标')
        # 接口名称，过滤规则，抓包数量，报文命名
        ...
        # 设备下发标记策略
        fun.send_cipso(tool, rbmDomain, rbmExc, clientOpeIp, serverOpeIp, mtLabel=self.case2_mtLabel, L4protocol='udp')

        # 服务端抓取报文
        log.warning('验证客户端发送含type7标记的udp报文')
        fun.client_send_server_pkt(cap_iface, cap_filter, cap_num, cap_pcap1, c_iface, c_num, c_pcap1, read_name1,
    ...
    @allure.feature('用例三：验证标记对udp报文的完整通信')
    def test_cipso_udp_double(self):
        log.warning('用例三：验证标记对udp报文的完整通信')
        # 接口名称，过滤规则，抓包数量，报文命名
        cap_iface, cap_filter, cap_num, cap_pcap1, cap_pcap2 = self.pkt3_cfg["capture"][0], self.pkt3_cfg["capture"][1],\
                                   self.pkt3_cfg["capture"][2], self.pkt3_cfg["capture"][3], self.pkt3_cfg["capture"][4]
        # 发送报文接口，发送报文数量，发送报文名称
        c_iface, c_num, c_pcap1, c_pcap2= self.pkt3_cfg["send"][0], self.pkt3_cfg["send"][1], self.pkt3_cfg["send"][2], self.pkt3_cfg["send"][3]
        read_name1, read_name2, read_id = self.pkt3_cfg["read"][0], self.pkt3_cfg["read"][1], self.pkt3_cfg["read"][2]
        # 设备下发标记策略
        fun.send_cipso(tool, rbmDomain, rbmExc, clientOpeIp, serverOpeIp, mtLabel=self.case1_mtLabel, L4protocol='udp')
        # 客户端--->服务端 的报文
        log.warning('验证客户端--->服务端的报文')
        fun.cmd(f"rm -rf /opt/pkt/{cap_pcap1}", 's')
        ...
        assert self.pkt3_cfg["expect"][0] == read_re

        # 服务端--->客户端 的报文
        log.warning('验证服务端--->客户端的报文')
        fun.cmd(f"rm -rf /opt/pkt/{cap_pcap2}", 'c')
        pre_cfg = fun.pkt_capture(cap_iface, cap_filter, cap_num, cap_pcap2)
        log.warning('客户端设置抓取报文：{}'.format(pre_cfg))
        fun.cmd(pre_cfg, 'c', thread=1)
        log.warning('step wait 20s')
        time.sleep(20)
        # 客户端发送报文
        send_cmd = fun.pkt_send(c_iface, c_num, c_pcap2)
        log.warning('服务端发送报文：{}'.format(send_cmd))
        fun.cmd(send_cmd, 's')
        # 检查报文是否存在
        ...
        log.warning('读取并分析报文：{}'.format(read_re))
        assert self.pkt3_cfg["expect"][1] == read_re
