...
try:
    from Case_rbm.iso_isolate_gapFromTo import index
...
class Test_iso_isolate_gapFromTo():
    ...
    def setup_class(self):
        ...
        self.case3_gapFromTo = index.case3_gapFromTo
        clr_env.iso_setup_class(dut='FrontDut')

    @allure.feature('用例一：验证隔离设备对隔离通道(为空)的检查')
    def test_iso_isolate_gapFromTo_none(self):
        # 下发配置
        log.warning('用例一：验证隔离设备对隔离通道(为空)的检查')
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='addhttp_front', GapFromTo=self.case1_gapFromTo), FrontDomain, base_path)
        time.sleep(3)
    ...
    @allure.feature('用例二：验证隔离设备对隔离通道(错误匹配)的检查')
    def test_iso_isolate_gapFromTo_fail(self):
        # 下发配置
        log.warning('用例二：验证隔离设备对隔离通道(错误匹配)的检查')
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='addhttp_front', GapFromTo=self.case2_gapFromTo), FrontDomain, base_path)
        time.sleep(3)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        front_res = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process', name='前置机nginx进程')
        assert front_res == 1
    ...
    @allure.feature('用例三：验证隔离设备对隔离通道(正确匹配)的检查')
    def test_iso_isolate_gapFromTo_ok(self):
        # 下发配置
        log.warning('用例三：验证隔离设备对隔离通道(正确匹配)的检查')
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='addhttp_front', GapFromTo=self.case3_gapFromTo), FrontDomain, base_path)
        time.sleep(3)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        front_res = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process', name='前置机nginx进程')
        assert front_res == 1
