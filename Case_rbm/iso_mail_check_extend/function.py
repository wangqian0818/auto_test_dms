...
class Test_iso_mail_check_extend():
    ...
    def setup_class(self):
        ...
        self.case1_extend1 = index.case1_extend1
        self.case2_extend1 = index.case2_extend1
        self.case2_extend2 = index.case2_extend2
    ...
    def test_iso_mail_check_extend_a1(self):
        ...
        log.warning('2、下发附件扩展名黑名单：txt，等待nginx的24个进程起来')
        fun.send(rbmExc,
                 tool.interface().app_safe_policy(prototype='mailcheck', attachmentExt_data=f'{self.case1_extend1}'),
        ...
        log.warning('黑名单文件扩展名{}结果为:{}'.format(self.case1_file1, result1))
        assert result1 == 0
        ...
        log.warning('非黑名单文件扩展名{}结果为:{}'.format(self.case1_file2, result2))
        assert result2 == 1
    ...
    def test_iso_mail_check_extend_a2(self):
        ...
        log.warning('2、下发附件扩展名黑名单：txt、pdf，等待nginx的24个进程起来')
        fun.send(rbmExc, tool.interface().app_safe_policy(prototype='mailcheck',
                                                        attachmentExt_data=f'{self.case2_extend1};{self.case2_extend2}'),
                 FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        add_check = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        assert add_check == 1
        ...
        log.warning('第一个黑名单文件扩展名{}结果为:{}'.format(self.case2_file1, result1))
        assert result1 == 0

        result2 = send_smtp.post_email(self.mail_sender, self.mail_receivers, self.mail_cc, self.mail_bcc,
                                       self.mail_host, self.mail_port, self.mail_user, self.mail_pass,
                                       self.case2_attach2, self.case2_file2, self.case2_title, self.context, 0, 1)
        log.warning('第二个黑名单文件扩展名{}结果为:{}'.format(self.case2_file2, result2))
        assert result2 == 0
        ...
        log.warning('非黑名单文件扩展名{}结果为:{}'.format(self.case2_file3, result3))
        assert result3 == 1
        ...
        # 检查邮件策略是否清空
        log.warning('移除邮件策略，等待nginx的24个进程起来')
        fun.send(rbmExc, tool.interface().app_safe_policy(prototype='delmailcheck'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        del_check = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        assert del_check == 1
        ...
        # 移除策略，还原环境
        log.warning('移除邮件的隔离策略，清空环境，等待nginx的24个进程起来')
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='delsmtp_front'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        fdel_res1 = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process', name='前置机nginx进程')
        assert fdel_res1 == 1
