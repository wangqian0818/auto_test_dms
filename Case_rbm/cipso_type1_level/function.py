...
'''
脚本一：
用例名称：验证type1标记的机密性等级（level）的左边界（最小值）
编写人员：刘超
编写日期：2021/11/17
测试目的：验证type1标记的机密性等级（level）的左边界（最小值）
测试步骤：
1.设备下发level为1的标记策略，通过babbitMQ下发
2.客户端tcpreplay发送level为1的报文
3.服务端抓取并分析报文
4.设备移除标记策略，通过babbitMQ删除
预期结果：
1.设备策略下发成功，可以使用tupleacl --get查看
2.客户端tcpreplay发送level为1的报文成功
3.服务端抓取分析成功
4.设备移除策略成功，可以使用tupleacl --get查看

脚本二 ：
用例名称：验证type1标记的机密性等级（level）的右边界（最大值）
编写人员：刘超
编写日期：2021/11/12
测试目的：验证type1标记的机密性等级（level）的右边界（最大值）
测试步骤：
1.设备下发level为255的标记策略，通过babbitMQ下发
2.客户端tcpreplay发送level为255的报文
3.服务端抓取并分析报文
4.设备移除标记策略，通过babbitMQ删除
预期结果：
1.设备策略下发成功，可以使用tupleacl --get查看
2.客户端tcpreplay发送level为255的报文成功
3.服务端抓取分析成功
4.设备移除策略成功，可以使用tupleacl --get查看
...
try:
    from Case_rbm.cipso_type1_level import index
...
class Test_cipso_type1_level():
    ...
    @allure.feature('用例一：验证type1标记的机密性等级（level）的左边界（最小值）')
    def test_cipso_level_minimum(self):
        log.warning('用例一：验证type1标记的机密性等级（level）的左边界（最小值）')
        # 设备设置level为1
        # 客户端发送level为1的报文
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例二：验证type1标记的机密性等级（level）的右边界（最大值）')
    def test_cipso_level_maximum(self):
        log.warning('用例二：验证type1标记的机密性等级（level）的右边界（最大值）')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例三：验证含type1标记策略的机密性（BLP模型）规则')
    def test_cipso_level_blp(self):
        log.warning('用例三：验证含type1标记策略的机密性（BLP模型）规则')
        # 接口名称，过滤规则，抓包数量，报文命名
        cap_iface, cap_filter, cap_num, cap_pcap1, cap_pcap2, cap_pcap3, cap_pcap4 = self.pkt3_cfg["capture"][0], \
                                                         self.pkt3_cfg["capture"][1],self.pkt3_cfg["capture"][2], \
                                                         self.pkt3_cfg["capture"][3], self.pkt3_cfg["capture"][4], \
                                                         self.pkt3_cfg["capture"][5], self.pkt3_cfg["capture"][6]
        # 发送报文接口，发送报文数量，发送报文名称
        c_iface, c_num, c_pcap1, c_pcap2, c_pcap3, c_pcap4 = self.pkt3_cfg["send"][0], self.pkt3_cfg["send"][1], \
                                                             self.pkt3_cfg["send"][2], self.pkt3_cfg["send"][3], \
                                                             self.pkt3_cfg["send"][4], self.pkt3_cfg["send"][5]
        read_name1, read_name2, read_name3, read_name4, read_id = self.pkt3_cfg["read"][0], self.pkt3_cfg["read"][1], \
                                                                  self.pkt3_cfg["read"][2], self.pkt3_cfg["read"][3], \
                                                                  self.pkt3_cfg["read"][4]
        # 设备下发标记策略
        fun.send_cipso(tool, rbmDomain, rbmExc, clientOpeIp, serverOpeIp, mtLabel=self.case3_mtLabel)
        # 服务端抓取报文
        log.warning('验证客户端发送level为13的报文')
        fun.client_send_server_pkt(cap_iface, cap_filter, cap_num, cap_pcap1, c_iface, c_num, c_pcap1, read_name1,
                                   read_id, self.pkt3_cfg["expect"][0])
        # 服务端抓取报文
        log.warning('验证客户端发送level为15的报文')
        fun.client_send_server_pkt(cap_iface, cap_filter, cap_num, cap_pcap2, c_iface, c_num, c_pcap2, read_name2,
                                   read_id, self.pkt3_cfg["expect"][0])

        # 服务端抓取报文
        log.warning('验证客户端发送level为16的报文')
        fun.client_send_server_pkt(cap_iface, cap_filter, cap_num, cap_pcap3, c_iface, c_num, c_pcap3, read_name3,
                                   read_id, self.pkt3_cfg["expect"][0])

        # 服务端抓取报文
        log.warning('验证客户端发送level为12的报文')
        fun.cmd(f"rm -rf /opt/pkt/{cap_pcap4}", 's')
        pre_cfg4 = fun.pkt_capture(cap_iface, cap_filter, cap_num, cap_pcap4)
        log.warning('服务端设置抓取报文：{}'.format(pre_cfg4))
        fun.cmd(pre_cfg4, 's', thread=1)
        log.warning('step wait 20s')
        time.sleep(20)
        # 客户端发送报文
        send_cmd = fun.pkt_send(c_iface, c_num, c_pcap4)
        log.warning('客户端发送报文：{}'.format(send_cmd))
        fun.cmd(send_cmd, 'c')
        # 检查报文是否存在
        pcap_file = fun.search('/opt/pkt', 'pcap', 's')
        fun.pid_kill(cap_pcap4)
        log.warning('服务端检查报文是否存在：{}'.format(pcap_file))
        assert cap_pcap4 not in pcap_file
