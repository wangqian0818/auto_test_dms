...
'''
脚本一：
用例名称：验证含type7标记策略的是转换模式时的规则
编写人员：刘超
编写日期：2021/11/12
测试目的：验证含type7标记策略的是转换模式时的规则
测试步骤：
1.设备下发转换标记策略，通过babbitMQ下发
2.客户端tcpreplay发送type7、cat为0x1,0x2,0x3,0x4的报文
3.服务端抓取并分析报文
4.设备移除标记策略，通过babbitMQ删除
预期结果：
1.设备策略下发成功，可以使用tupleacl --get查看
2.客户端tcpreplay发送type7、cat为0x1,0x2,0x3,0x4的报文成功
3.服务端抓取分析报文type1、cat为0x1,0x0,0x0,0x0成功
4.设备移除策略成功，可以使用tupleacl --get查看

脚本二：
用例名称：验证含type7标记策略的是反向会话时的规则
编写人员：刘超
编写日期：2021/11/12
...
1.设备策略下发成功，可以使用tupleacl --get查看
2.客户端tcpreplay发送type7、cat为0x1,0x2,0x3,0x4的报文成功
3.客户端抓取分析报文type7、cat为0x1,0x0,0x0,0x0成功
4.设备移除策略成功，可以使用tupleacl --get查看
...
try:
    from Case_rbm.cipso_type7_markaction import index
...
class Test_cipso_type7_markaction():
    ...
    @allure.feature('用例一：验证含type7标记策略的是转换模式时的规则')
    def test_cipso_type7_change(self):
        log.warning('用例一：验证含type7标记策略的是转换模式时的规则')
        # 设备设置DOI为1
        # 客户端发送DOI为1的报文
        # 接口名称，过滤规则，抓包数量，报文命名
        ...
        # 设备下发标记策略
        fun.send_cipso(tool, rbmDomain, rbmExc, clientOpeIp, serverOpeIp, mtLabel=self.case1_mtLabel, tgLabel=self.case1_tgLabel)
        # 服务端抓取报文
    ...
    @allure.feature('用例二：验证含type7标记策略的是反向会话时的规则')
    def test_cipso_type1_rechange(self):
        log.warning('用例二：验证含type7标记策略的是反向会话时的规则')
        # 设备设置DOI为1
        # 客户端发送DOI为1的报文
        # 接口名称，过滤规则，抓包数量，报文命名
