...
测试步骤：
1.设备下发Match为0、cat为0x3,0x2,0x3,0x4的标记策略，通过babbitMQ下发
...
脚本五：
用例名称：验证type7类型下的类别左边界（最小）值
编写人员：刘超
编写日期：2021/11/12
测试目的：验证type7类型下的类别左边界（最小）值
测试步骤：
...
脚本六：
用例名称：验证type7类型下的类别右边界（最大）值
编写人员：刘超
编写日期：2021/11/12
测试目的：验证type7类型下的类别右边界（最大）值
测试步骤：
...
try:
    from Case_rbm.cipso_type7_cat import index
...
class Test_cipso_type7_cat():
    ...
    @allure.feature('用例一：验证含type7标记策略的类别在子集比对方式下的规则')
    def test_cipso_type7_cat_inc(self):
        log.warning('用例一：验证含type7标记策略的类别在子集比对方式下的规则')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例二：验证含type7标记策略的类别在交集比对方式下的规则')
    def test_cipso_type7_cat_1bit(self):
        log.warning('用例二：验证含type7标记策略的类别在交集比对方式下的规则')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例三：验证含type7标记策略的类别在非比对方式下的规则')
    def test_cipso_type7_cat_neq(self):
        log.warning('用例三：验证含type7标记策略的类别在非比对方式下的规则')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例四：验证含type7标记策略的类别在等于比对方式下的规则')
    def test_cipso_type7_cat_eq(self):
        log.warning('用例四：验证含type7标记策略的类别在等于比对方式下的规则')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例五：验证type7类型下的类别左边界（最小）值')
    def test_cipso_type7_cat_minimum(self):
        log.warning('用例五：验证type7类型下的类别左边界（最小）值')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例六：验证type7类型下的类别右边界（最大）值')
    def test_cipso_type7_cat_maximum(self):
        log.warning('用例六：验证type7类型下的类别右边界（最大）值')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例七：验证type7类型下的类别为f000时，对报文的处理原则')
    def test_cipso_type7_cat_f000(self):
        log.warning('用例七：验证type7类型下的类别为f000时，对报文的处理原则')
        # 接口名称，过滤规则，抓包数量，报文命名
        cap_iface, cap_filter, cap_num, cap_pcap1, cap_pcap2 = self.pkt7_cfg["capture"][0], self.pkt7_cfg["capture"][1], \
                                                               ...
                                                               self.pkt7_cfg["capture"][4]
        # 发送报文接口，发送报文数量，发送报文名称
        c_iface, c_num, c_pcap1, c_pcap2 = self.pkt7_cfg["send"][0], self.pkt7_cfg["send"][1], self.pkt7_cfg["send"][2], \
                                           self.pkt7_cfg["send"][3]
    ...
    @allure.feature('用例八：验证type7类型下的类别为ff00时，对报文的处理原则')
    def test_cipso_type7_cat_ff00(self):
        log.warning('用例八：验证type7类型下的类别为ff00时，对报文的处理原则')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例九：验证type7类型下的类别为fff0时，对报文的处理原则')
    def test_cipso_type7_cat_fff0(self):
        log.warning('用例九：验证type7类型下的类别为fff0时，对报文的处理原则')
        # 接口名称，过滤规则，抓包数量，报文命名
    ...
    @allure.feature('用例十：验证type7类型下的类别为0fff时，对报文的处理原则')
    def test_cipso_type7_cat_0fff(self):
        log.warning('用例十：验证type7类型下的类别为0fff时，对报文的处理原则')
        # 接口名称，过滤规则，抓包数量，报文命名
        ...
        read_name1, read_name2, read_id = self.pkt10_cfg["read"][0], self.pkt10_cfg["read"][1], self.pkt10_cfg["read"][
            2]
        # 1.---------------------------测试去标---------------------------------
        log.warning('去标测试')
        # 设备下发标记策略
    ...
    @allure.feature('用例十一：验证type7类型下的类别为f0ff时，对报文的处理原则')
    def test_cipso_type7_cat_f0ff(self):
        log.warning('用例十一：验证type7类型下的类别为f0ff时，对报文的处理原则')
        # 接口名称，过滤规则，抓包数量，报文命名
        ...
        read_name1, read_name2, read_id = self.pkt11_cfg["read"][0], self.pkt11_cfg["read"][1], self.pkt11_cfg["read"][
            2]
        # 1.---------------------------测试去标---------------------------------
        log.warning('去标测试')
        # 设备下发标记策略
    ...
    @allure.feature('用例十二：验证type7类型下的类别为ff0f时，对报文的处理原则')
    def test_cipso_type7_cat_ff0f(self):
        log.warning('用例十二：验证type7类型下的类别为ff0f时，对报文的处理原则')
        # 接口名称，过滤规则，抓包数量，报文命名
        ...
        read_name1, read_name2, read_id = self.pkt12_cfg["read"][0], self.pkt12_cfg["read"][1], self.pkt12_cfg["read"][
            2]
        # 1.---------------------------测试去标---------------------------------
        log.warning('去标测试')
        # 设备下发标记策略
