# encoding='utf-8'
from data_check import send_smtp, recv_pop3, con_ftp, http_check
...
print(base_path)
try:
    from Case_rbm.iso_tcp_keyword import index
...
class Test_iso_tcp_keyword():
    ...
    def teardown_method(self):
        fun.send(rbmExc, tool.interface().keyword_interface(appid=smtp_appid, ruleid=4), FrontDomain, base_path)
        fun.send(rbmExc, tool.interface().keyword_interface(appid=pop3_appid, ruleid=1), FrontDomain, base_path)
        fun.send(rbmExc, tool.interface().keyword_interface(appid=ftp_appid, ruleid=self.ftp_ruleid), FrontDomain,
                 base_path)
        fun.send(rbmExc, tool.interface().keyword_interface(appid=tcp_appid, ruleid=self.tcp_ruleid), FrontDomain,
                 base_path)
    ...
    def test_iso_mail_keyword(self):
        # 下发配置
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='add_smtp_front'), FrontDomain, base_path)
        ...
        assert back_res1 == 1
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='add_pop3_front'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        ...
        fun.send(rbmExc, tool.interface().keyword_interface(appid=pop3_appid,
                                                            ...
                                                            spattern=f'{self.pop3_keyword}'), FrontDomain, base_path)
        log.warning('检查内容审查策略是否下发成功')
        time.sleep(10)
        re = fun.wait_data(type=8, dut='FrontDut')
        ...
        # 移除策略，清空环境
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='del_smtp_front'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        ...
        assert bdel_res1 == 1
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='del_pop3_front'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
    ...
    def test_iso_ftp_keyword(self):
        # 下发配置
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='add_ftp_front'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        ...
        # 移除策略，清空环境
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='del_ftp_front'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
    ...
    def test_iso_tcp_keyword(self):
        # 下发配置
        fun.send(rbmExc, tool.interface().setAccessconf(prototype='add_tcp_http_front'), FrontDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        front_res = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process', name='前置机nginx进程')
        assert front_res == 1
        fun.wait_data('ps -ef |grep nginx', 'BackDut', 'nginx: worker process')
